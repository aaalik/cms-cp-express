...
const query = async(text) => {
    ...
    .catch(() => 'Database connected', err => new Error(`Database error! ${err}`));;
    return rows;
};
...
const updateRow = async(tableField, tableValue, response) => {
    try{
        const baseSql = `UPDATE ${tableField} SET ?`
        const rows = await connection().query(baseSql, tableValue);
        return rows;
    }catch(err){
        return response.status(500).send(err)
...
export { connection, testConnect, query, insertRow };
